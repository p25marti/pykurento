...
class MultiResHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        MultiResHandler.low_res = media.GStreamerFilter(
            pipeline, command="capsfilter caps=video/x-raw,width=160,height=120", filterType="VIDEO")
        MultiResHandler.med_res = media.GStreamerFilter(
            pipeline, command="capsfilter caps=video/x-raw,width=320,height=240", filterType="VIDEO")
